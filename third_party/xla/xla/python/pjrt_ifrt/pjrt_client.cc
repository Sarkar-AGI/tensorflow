...
tsl::Future<std::vector<uint64_t>> PjRtClient::HashValues(
  ...
  auto [promise, future] = tsl::MakePromise<std::vector<uint64_t>>();
  tsl::Env::Default()->SchedClosure(
      [promise = std::move(promise),
       // Keep values alive until hashing is complete.
       values = std::vector<ValueRef>(values.begin(), values.end()),
       mode]() mutable {
        auto result = [&]() -> absl::StatusOr<std::vector<uint64_t>> {
          // Number of buffers for each value. This is the length of the
          // contiguous span in `flat_buffers` and `flat_index_domains` that
          // correspond to each value.
          std::vector<int> num_buffers_per_value;
          num_buffers_per_value.reserve(values.size());
          // Flat list of all buffers and index domains for all values. This
          // allows applying the hashing and throttling logic in a single call
          // to `HashPjRtBuffers()`.
          ...
          ASSIGN_OR_RETURN(
              ...
              HashPjRtBuffers(flat_buffers, flat_index_domains, mode));
          ...
          for (int i = 0; i < values.size(); ++i) {
            ...
            absl::Span<const uint64_t> value_shard_hashes =
                absl::MakeSpan(shard_hashes.data() + flat_index, num_buffers);
            absl::Span<const IndexDomain> value_index_domains = absl::MakeSpan(
                flat_index_domains.data() + flat_index, num_buffers);
