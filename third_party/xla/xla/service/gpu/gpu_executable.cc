...
absl::flat_hash_map<std::string, const HloInstruction*> MakeConstantsMap(
  ...
  for (const HloComputation* computation :
    ...
    for (const HloInstruction* instr : computation->instructions()) {
      ...
      constants[llvm_ir::ConstantHloToGlobalName(*instr)] = instr;
    }
  }
  return constants;
}
...
GpuExecutable::ConstantInfo::FromProto(
    ...
    std::optional<absl::flat_hash_map<std::string, const HloInstruction*>>
        overrides) {
  if (overrides) {
    auto it = overrides->find(proto.symbol_name());
    if (it == overrides->end()) {
...
absl::StatusOr<std::unique_ptr<GpuExecutable>> GpuExecutable::FromProto(
  ...
  *thunk_sequence_proto.mutable_thunks() = proto.thunks();
  ...
  std::optional<absl::flat_hash_map<std::string, const HloInstruction*>>
      name_to_const;
  if (HloModule* debug_module = params.debug_module.get(); debug_module) {
    name_to_const = MakeConstantsMap(debug_module);
  }
