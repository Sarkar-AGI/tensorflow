...
absl::StatusOr<std::vector<ShapeTracker::BufferView>> SliceProjectionChain(
  ...
  if (expected_elements == 1) {
    ASSIGN_OR_RETURN(ShapeTracker::BufferView scalar_view,
                     ShapeTracker::BufferView::FromStridesAndExtents({1}, {1}));
    std::vector<ShapeTracker::BufferView> sliced_projections(projections.size(),
                                                             scalar_view);
  ...
  for (size_t i = 0; i < projections.size(); ++i) {
    // Merge dimensions to simplify intersection testing.
    current_slice.MergeAdjacentDimensions();
